...
////////////////////////////////////////////////////////////SET WAYPOINTS ON RVIZ////////////////////////////////
    ...
    while (set_goals)
    {
        shared_goal = ros::topic::waitForMessage<geometry_msgs::PoseStamped>("/move_base_simple/goal", nh_, ros::Duration(1.0)); ///// TEMP 1s/////
        if (shared_goal == nullptr)
        {
            if (goal_nodes_.empty())
            {
                default_way_points.resize(7, 2);
                default_way_points << 2.0 ,  0.0,
                                      2.0 , -2.0,
                                      0.0 , -2.0,
                                      0.0 , -1.0,
                                      2.0 , -1.0,
