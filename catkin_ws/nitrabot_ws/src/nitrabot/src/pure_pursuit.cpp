...
bool PurePursuit::nearGoal(const Eigen::Vector3d &error) const
{   
    if(std::abs(error(2))>6.0)
    {
        if (std::abs(error(0)) < POS_THRESHOLD2 && std::abs(error(1)) < POS_THRESHOLD2 && (std::abs(error(2)) - MathConstants::TWOPI) < HEADING_THRESHOLD2)
        {
            return true;
        }
    }
    if (std::abs(error(0)) < POS_THRESHOLD2 && std::abs(error(1)) < POS_THRESHOLD2 && std::abs(error(2)) < HEADING_THRESHOLD2)
    {
        return true;
    }
    return false;
...
bool PurePursuit::nearGoal2(const Eigen::Vector3d &error) const
{   
    if(std::abs(error(2))>6.0)
    {
        if (std::abs(error(0)) < POS_THRESHOLD3 && std::abs(error(1)) < POS_THRESHOLD3 && (std::abs(error(2)) - MathConstants::TWOPI) < HEADING_THRESHOLD3)
        {
            return true;
        }
    }
    if (std::abs(error(0)) < POS_THRESHOLD3 && std::abs(error(1)) < POS_THRESHOLD3 && std::abs(error(2)) < HEADING_THRESHOLD3)
    {
        return true;
    }
    return false;
...
void PurePursuit::rpm_convert_publish(Eigen::Vector2d &input)
{
    ...
    std::cout << "vel msg:"<<  velocity_msgs_.data[0] << "," << velocity_msgs_.data[1]<<std::endl;
