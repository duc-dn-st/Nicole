...
//*/
/*
        this->bupineq_(ii, 0) = max_vel_ - v_ref_k0;
        this->bupineq_(ii+HORIZON-1, 0) = max_steer_angle_ - delta_ref_k0;
        this->bupineq_(ii+(HORIZON-1)*2, 0) = max_acc_ - (v_ref_k1 - v_ref_k0);
        this->bupineq_(ii+(HORIZON-1)*3, 0) = max_steer_vel_ - (delta_ref_k1 - delta_ref_k0);

        this->blowineq_(ii, 0) = -max_vel_ - v_ref_k0;
        this->blowineq_(ii+HORIZON-1, 0) = -max_steer_angle_ - delta_ref_k0;
        this->blowineq_(ii+(HORIZON-1)*2, 0) = -max_acc_ - (v_ref_k1 - v_ref_k0);
        this->blowineq_(ii+(HORIZON-1)*3, 0) = -max_steer_vel_ - (delta_ref_k1 - delta_ref_k0);
*/
    }    
}
...
void MpcAckerman::build_mpc_cost()
{
    ...
    Q.diagonal() << 25.0, 25.0, 50.0;    // 80 80
...
void MpcAckerman::calculate_opt_control(const std::vector<Eigen::Vector3d> &pose_ref_traj, const std::vector<Eigen::Vector2d> &input_ref_traj)
{
    ...
    for (long ii = 0; ii < HORIZON; ++ii)
    {
        ...
        //std::cout << "pose" << pose_ref << std::endl;
        //std::cout << "input" << input_ref << std::endl;

        Eigen::Matrix3d lin_A = get_A_mpc(pose_ref, input_ref);
        Eigen::Matrix<double,3,2> lin_B = get_B_mpc(pose_ref, input_ref);

        // std::cout << "A:" << std::endl << lin_A << std::endl;
        // std::cout << "B:" << std::endl << lin_B << std::endl;
...
void MpcAckerman::generateCSV(const std::vector<Eigen::Vector3d> &pose_ref_traj, const std::vector<Eigen::Vector2d> &input_ref_traj)
{
   ...
   // Eigen::Vector3d error = pose_ - this->ref_.block(1, this->counter_, NUM_STATE, 1);
   // Eigen::Vector3d state_ref = this->ref_.block(1, this->counter_, NUM_STATE, 1);
...
/*
    Eigen::Matrix3d rot;
    std::vector<Eigen::Vector3d> pose_ref_traj_r(HORIZON);
    
    for(long i=0; i<HORIZON; i++)
    {
        rot << std::cos(pose_ref_traj.at(i).z()), -std::sin(pose_ref_traj.at(i).z()), 0,
               std::sin(pose_ref_traj.at(i).z()), std::cos(pose_ref_traj.at(i).z()),  0,
               0,   0,  1;
        pose_ref_traj_r.at(i) = rot * pose_ref_traj.at(i);
    }
 */       
   ...
   // pose_ = rot * robot_pose;
    ...
    //input_wmr_(0)=this->ref_(INPUT_startCOL_, counter_);
    //input_wmr_(1)=this->ref_(INPUT_startCOL_ +1 , counter_);
   ...
   if (GeneralFunctions::isEqual(input_wmr_(1), 0.0))
    ...
    else
    {
        ...
        double outer_steering = std::atan(RobotConstants::FRONT_TO_REAR_WHEEL / (std::abs(turn_radius) + RobotConstants::AXLE_LENGTH / 2));
        double inner_steering = std::atan(RobotConstants::FRONT_TO_REAR_WHEEL / (std::abs(turn_radius) - RobotConstants::AXLE_LENGTH / 2));
   ...
   // std::cout << "time for log" << duration << std::endl;
    // check if at goal
    //if (atGoal(error))
    //{
