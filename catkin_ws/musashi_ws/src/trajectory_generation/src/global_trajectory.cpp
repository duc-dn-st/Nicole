...
Global_traj_class::Global_traj_class(ros::NodeHandle nh, ros::NodeHandle private_nh)
{
    ...
    astar_path_.clear();
    nav_msgs::PathConstPtr astar_msg;
    astar_msg = ros::topic::waitForMessage<nav_msgs::Path>("/path_planner/a_star_path", nh_, ros::Duration(30.0));
    if (astar_msg == nullptr)
    {
        ROS_ERROR("No A* path received.");
    }
    else
    {
        Eigen::Vector3d pose;
        for (unsigned int ii = 0; ii < astar_msg->poses.size(); ii++)
        {
            pose << astar_msg->poses[ii].pose.position.x, astar_msg->poses[ii].pose.position.y, tf::getYaw(astar_msg->poses[ii].pose.orientation);
            astar_path_.push_back(pose);
...
void Global_traj_class::export_global_trajectory() {
    ...
    export_file.open("global_trajectory.csv",std::ios::out|std::ios::trunc);

    // Write data into the file
    export_file << "Time [s], X-Pos [m], Y-Pos [m], phi [rad], dX-Pos [m/s], dY-Pos [m/s], dphi [rad/s], Path Vel [m/s]" << std::endl;
    //	export_file << ref_traj << std::endl;

    for (unsigned int i = 0; i < Ns; i++)
    {
        export_file << t(i) << "," << pos(i, 0) << "," << pos(i, 1) << "," << phi(i) << "," << vel(i, 0) << "," << vel(i, 1) << "," << dphi(i) << "," << path_vel(i) << std::endl;
    }
    // Close the file
    export_file.close();
}
