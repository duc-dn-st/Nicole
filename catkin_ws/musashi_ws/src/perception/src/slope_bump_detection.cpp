/*
 * ===============================================================================
 * slope_bump_detection.h
 * Author: Schaefle Tobias
 * Date: 14.07.20
 * Email: tobias.schaefle@gmail.com
 * -------------------------------------------------------------------------------
 * Description:
 * This class reads point cloud data from the velodyne sensor (currently only front)
 * and checks if there is a slope or bump along the robot trajectory.
 * ===============================================================================
 */

#include "../include/perception/slope_bump_detection.h"

void SlopeBumpDetection::frontVelodynePointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr &front_velo_msg)
{
    //std::cout << "frame:\t" << front_velo_msg->header.frame_id << std::endl;

    // convert cloud
    pcl::PCLPointCloud2 pcl_pc2;
    pcl_conversions::toPCL(*front_velo_msg, pcl_pc2);
...
void SlopeBumpDetection::odometryCallback(const nav_msgs::OdometryConstPtr odom_msg)
{
...
void SlopeBumpDetection::publishSlopeBumpMap()
{
    // updatet map pose
    updateMapInfo();
    ...
    if (cropped_cloud_->size() == 0)
    {
        ...
        pub_slope_map_.publish(slope_map_msg_);
        return;
    }
    ...
    for (unsigned int kk = 0; kk < cropped_cloud_->size(); ++kk)
    {
        ...
        cell_x = static_cast<int>(coordinates.x() / slope_map_msg_.info.resolution);
        cell_y = static_cast<int>(coordinates.y() / slope_map_msg_.info.resolution);
        // map index
        index = static_cast<unsigned int>(cell_x) + slope_map_msg_.info.width * static_cast<unsigned int>(cell_y);
        if (inGrid(cell_x, cell_y))
        {
            slope_map_msg_.data[index] = 50;
        }
    }
    */
    ...
    for (unsigned int ii = 0; ii < cloud_normals->size(); ++ii)
    {
        ...
        // map index
        index = static_cast<unsigned int>(cell_x) + slope_map_msg_.info.width * static_cast<unsigned int>(cell_y);
        if (!inGrid(cell_x, cell_y) || slope_map_msg_.data[index] == 100)
        {
        ...
        else
        {
            ...
            {
                // violation
                slope_map_msg_.data[index] = 100;
                x = coordinates.x();
                y = coordinates.y();
                // inflate obstacle
                double radius = slope_map_msg_.info.resolution;
                ...
                while (radius <= ROBOT_RADIUS)
                {
                    ...
                    radius += slope_map_msg_.info.resolution;
                }
            }
        }
    }
    ...
    */
    ...
    pub_slope_map_.publish(slope_map_msg_);
    // reset map
    for (unsigned int kk = 0; kk < slope_map_msg_.info.width * slope_map_msg_.info.height; ++kk)
    {
        slope_map_msg_.data[kk] = 0;
    }

}

void SlopeBumpDetection::updateMapInfo()
{   
    try
    {
        listener_->lookupTransform("/odom", "/base_link", ros::Time(0), transform_odom_base_);
        listener_->lookupTransform("/base_link", "/velodyne_front", ros::Time(0), transform_base_velo_);
    ...
    tf::Transform transform_odom_velo = transform_odom_base_ * transform_base_velo_;

    double roll, pitch, yaw;
    //transform_odom_velo.getBasis().getRPY(roll, pitch, yaw);
    transform_odom_base_.getBasis().getRPY(roll, pitch, yaw);

    //ROS_INFO("%f", yaw);
    map_metadata_msg_.origin.position.x = transform_odom_velo.getOrigin().getX()
        - sin(yaw) * (-double(map_metadata_msg_.resolution) * map_metadata_msg_.height/2 - transform_base_velo_.getOrigin().getY());
    map_metadata_msg_.origin.position.y = transform_odom_velo.getOrigin().getY()
        + cos(yaw) * (-double(map_metadata_msg_.resolution) * map_metadata_msg_.height/2 - transform_base_velo_.getOrigin().getY());
    map_metadata_msg_.origin.position.z = (transform_odom_velo.getOrigin().getZ() - 0.10635); // ground
    ...
    slope_map_msg_.info = map_metadata_msg_;
...
void SlopeBumpDetection::transformCloudPoints(const pcl::PointXYZ &point, Eigen::Vector3d &coordinates)
{
    tf::Transform orig_tf, point_tf;
    tf::poseMsgToTF(slope_map_msg_.info.origin, orig_tf);
    ...
    point_tf = orig_tf.inverse() * transform_odom_base_ * transform_base_velo_ * point_tf;
...
inline bool SlopeBumpDetection::inGrid(int cell_x, int cell_y)
{
    if ((cell_x < 0 || cell_x >= static_cast<signed int>(slope_map_msg_.info.width))
	        || (cell_y < 0 || cell_y >= static_cast<signed int>(slope_map_msg_.info.height)))
	{
	    return false;
	}
	else {return true;}
...
void SlopeBumpDetection::updateAllOctants(double x, double y, double *obstacle_x, double *obstacle_y, signed char inflated_value)
{
...
void SlopeBumpDetection::midpointCircleAlgorithm(double obstacle_x, double obstacle_y, double radius, signed char inflate_cost)
{
	...
	while (x > y)
	{
	    ...
	    x = sqrtf(x * x - 2 * y * slope_map_msg_.info.resolution - (slope_map_msg_.info.resolution * slope_map_msg_.info.resolution));
	    // y+1^2 = (y + 1)^2 (1px) -> y+1 = y + 1px
	    y = y + slope_map_msg_.info.resolution;

	    updateAllOctants(x, y, &obstacle_x, &obstacle_y, inflate_cost);
	}
}

void SlopeBumpDetection::updateInflatedValues(double x, double y, signed char inflate_value)
{
	// get cell pos x and y
	int cell_x = static_cast<int>(x / slope_map_msg_.info.resolution);
    int cell_y = static_cast<int>(y / slope_map_msg_.info.resolution);

	// map index
	unsigned int index = static_cast<unsigned int>(cell_x) + slope_map_msg_.info.width * static_cast<unsigned int>(cell_y);
	if (inGrid(cell_x, cell_y))
	{
	    // check if cell is already assigned with a higher value
	    if (slope_map_msg_.data[index] < inflate_value)
	        slope_map_msg_.data[index] = inflate_value;
	}
}
