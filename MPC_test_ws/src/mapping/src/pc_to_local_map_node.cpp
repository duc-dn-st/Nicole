...
private:
    ...
    // Other vars
    /*
    const signed char INFLATE_ADDITIONAL_LAYER_COST = 10;
    const signed char INFLATE_DIAGONAL_COST = 25;
    const signed char INFLATE_LENGTH_COST = 50;
    const signed char INFLATE_WIDTH_COST = 75;

    const float ADDITIONAL_RADIUS = 0.9f;
    const float DIAGONAL_RADIUS = 0.808f;
    const float LENGTH_RADIUS = 0.650f;
    const float WIDTH_RADIUS = 0.480f;
    */
    ...
    const float ROBOT_RADIUS = 0.808f;
    const float RECTANGLE_RADIUS = 0.509f;
    const float SQUARE_RADIUS = 0.340f;
    ...
    */
    ...
    void midpointCircleAlgorithm(float obstacle_x, float obstacle_y, float radius, signed char inflate_cost)
    {
        ...
        while (x > y)
        {
            ...
            x = sqrtf(x * x - 2 * y * local_map_msg_.info.resolution - (local_map_msg_.info.resolution * local_map_msg_.info.resolution));
            // y+1^2 = (y + 1)^2 (1px) -> y+1 = y + 1px
            y = y + local_map_msg_.info.resolution;
    ...
    void updateInflatedValues(float x, float y, signed char inflate_value)
    {
        ...
        int cell_x = int(round(double(x / local_map_msg_.info.resolution)));
        int cell_y = int(round(double(y / local_map_msg_.info.resolution)));

        // map index
        unsigned int index = static_cast<unsigned int>(cell_x) + local_map_msg_.info.width * static_cast<unsigned int>(cell_y);
        if (inGrid(cell_x, cell_y))
        {
            // check if cell is already assigned with a higher value
            if (local_map_msg_.data[index] < inflate_value)
                local_map_msg_.data[index] = inflate_value;
        }
    }
...
public:
    ...
    */
        ...
        for (sensor_msgs::PointCloud2ConstIterator<float> iter_x(*velo_front_, "x"); iter_x != iter_x.end(); ++iter_x)
        {
            ...
            if (coordinates[2] > 0.01f && !inRobot)
            {
                // get cell pos x and y
                ...
                if (inGrid(cell_x, cell_y))
                {
                    // if local map does not have an obstacle in that cell than define it and inflate the obstacle otherwise do nothing
                    if (local_map_msg_.data[index] != 100)
                    {
                        local_map_msg_.data[index] = 100;
                        //std::cout << "Point Coordinates:\t[" << x << ", " << y << ", " << z << "]" << std::endl;
                        // inflate obstacle
                        float radius = local_map_msg_.info.resolution;
                        /*
            while (radius <= ADDITIONAL_RADIUS)
            {
                if (radius <= WIDTH_RADIUS)
                {
                midpointCircleAlgorithm(x, y, radius, INFLATE_WIDTH_COST);
                }
                else if (radius <= LENGTH_RADIUS)
                {
                midpointCircleAlgorithm(x, y, radius, INFLATE_LENGTH_COST);
                }
                else if (radius <= DIAGONAL_RADIUS)
                {
                midpointCircleAlgorithm(x, y, radius, INFLATE_DIAGONAL_COST);
                }
                else
                {
                midpointCircleAlgorithm(x, y, radius, INFLATE_ADDITIONAL_LAYER_COST);
                }
                radius += local_map_msg_.info.resolution;
            }
            */
                        while (radius <= ROBOT_RADIUS)
                        {
                            ...
                            {
                                midpointCircleAlgorithm(coordinates[0], coordinates[1], radius, INFLATE_SQUARE_COST);
                            }
                            else if (radius <= RECTANGLE_RADIUS)
                            {
                                midpointCircleAlgorithm(coordinates[0], coordinates[1], radius, INFLATE_RECTANGLE_COST);
                            }
                            else
                            {
                                midpointCircleAlgorithm(coordinates[0], coordinates[1], radius, INFLATE_ROBOT_COST);
                            }
                            radius += local_map_msg_.info.resolution;
                        }
                    }
