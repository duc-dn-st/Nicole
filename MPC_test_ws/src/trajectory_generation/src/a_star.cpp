...
void Astar::initializeMessages()
{
    // get map
    ROS_INFO("Set Map.");
    static_map_ = ros::topic::waitForMessage<nav_msgs::OccupancyGrid>("inflate_static_map_node/inflated_map", nh_, ros::Duration(5.0));
    ...
    while (set_goals)
    {
        shared_goal = ros::topic::waitForMessage<geometry_msgs::PoseStamped>("/move_base_simple/goal", nh_, ros::Duration(3.0));
