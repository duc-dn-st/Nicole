...
#ifndef SLOPE_BUMP_DETECTION_H
#define SLOPE_BUMP_DETECTION_H
...
class SlopeBumpDetection
{
public:
    SlopeBumpDetection(ros::NodeHandle nh, ros::NodeHandle private_nh)
    {
        nh_ = nh;
        ...
        pub_slope_map_ = private_nh_.advertise<nav_msgs::OccupancyGrid>("slope_map", 1);
        pub_cloud_test_ = private_nh_.advertise<sensor_msgs::PointCloud2>("test_crop", 1);

        nh_.param("map_width", map_width_, 2.5);
        nh_.param("map_height", map_height_, 1.0);
	    ...
	    map_metadata_msg_.origin.position.x = 0 + transform_base_velo_.getOrigin().getX();
	    map_metadata_msg_.origin.position.y = 0 - map_height_ / 2;
        map_metadata_msg_.origin.position.z = (transform_odom_base_ * transform_base_velo_).getOrigin().getZ() - 0.10635; // ground
        map_metadata_msg_.origin.orientation.w = 1;

        slope_map_msg_.info = map_metadata_msg_;
        slope_map_msg_.data.resize(map_metadata_msg_.width * map_metadata_msg_.height);
        slope_map_msg_.header.frame_id = "odom";

        for (unsigned long kk = 0; kk < slope_map_msg_.data.size(); ++kk)
        {
...
*/
        ...
        double additional_translation = (map_height_ / 2 - transform_base_velo_.getOrigin().getY() / 2) / 2;
        Eigen::Vector3f translation(0, -transform_base_velo_.getOrigin().getY() - additional_translation, 0);
        double roll, pitch, yaw;
        transform_base_velo_.getBasis().getRPY(roll, pitch, yaw);
    ...
    void frontVelodynePointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr &front_velo_msg);

    void odometryCallback(const nav_msgs::OdometryConstPtr odom_msg);

    void publishSlopeBumpMap();
